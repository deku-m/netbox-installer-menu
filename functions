...
os_check() {
 if [[ $(lsb_release -d) == *20.04* ]]; then
   ...
   echo -e "-----------------------------------------------------------------------"
   echo -e "$(color_red 'You are not running Ubuntu 20.04. Installation cancelled.')"
   echo -e "-----------------------------------------------------------------------"
   exit 1
fi
}
...
echo -e "----------------------------------------------------------------------------------"
echo -e "$(color_red 'Dont forget the SSL cert! /etc/nginx/sites-enabled/netbox.conf')" 
echo -e "======================================================================================"
}
...
full_backup() {
  ...
  echo -e "----------------------------------------------------"
  mkdir /opt/netbox/backups/database/
  ...
  echo -e "Enter filename $(color_red '"full_backup"') and press Enter:"
  read -r full_bck_file
  ...
  echo -e "----------------------------------------------------------"
  touch /opt/netbox/netbox/netbox/full_backup.sh
...
schema_only_backup() {
  ...
  echo "Fill in the information of SFTP sever"
  echo -e "Enter filename $(color_red 'schema_backup') and press Enter:"
  read -r sm_bck_file
  echo -e "Enter SFTP server IP:"
  read -r sftp_host
  echo -e "Enter username:"
  ...
  echo -e "----------------------------------------------------------"
  touch /opt/netbox/netbox/netbox/schema_backup.sh
...
echo -e "----------------------------------------------------"
echo -e "Enter the pluginname and press Enter:"
read -r pluginname
echo -e "----------------------------------------------------"
...
update_plugin() {
echo -e "----------------------------------------------------"
echo -e "$(color_yellow 'Update plugin...')"
echo -e "----------------------------------------------------"
echo -e "Enter the pluginname and press Enter:"
read -r pluginname
echo -e "----------------------------------------------------"
echo -e "$(color_green 'Plugin update completed')"
echo -e "----------------------------------------------------"
  if [ "$?" -gt "0" ];
    then
      echo -e "----------------------------------------------------------"
      echo -e "$(color_red 'Plugin not updated successful.')"
      echo -e "----------------------------------------------------------"
   exit 1
  fi
}
